...
exports.applyJob = async (req, res) => {
   ...
   try {
      ...
      if (job.applyCount > 0) {
         if (job.appliedCandidate.map((jid) => jid.applicantId.toString() === applicantId.toString())[0]) {
      ...
      if (expired) {
         return res.status(500).json({ status: "Failed", error: "Deadline for applying this job is expired" });
      }
